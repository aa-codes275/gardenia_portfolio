...
function renderWorks(filter = "all") {
  ...
  grid.innerHTML = items.map((w, i) => `
    <div class="tile reveal in" data-i="${WORKS.indexOf(w)}">
      ${w.type === "video"
        ? `<video src="${w.src}" muted playsinline preload="metadata" disablepictureinpicture></video><div class="ov"></div><div class="play"><span><i class="fa-solid fa-play"></i></span></div>`
        : `<img src="${w.src}" alt="${w.t}" loading="lazy" decoding="async"/><div class="ov"></div>`}
...
function openLb(w) {
  ...
  lb.querySelector("#lbMedia").innerHTML = w.type === "video"
    ? `<video src="${w.src}" controls autoplay playsinline preload="auto"></video>`
...
document.addEventListener("click", e => {
  if (e.target.id === "lb" || e.target.id === "lbClose" || e.target.closest("#lbClose")) closeLb();
});
